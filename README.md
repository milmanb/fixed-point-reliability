...
Generated by `scripts/report_table.py` (`results/report_table.csv`). Spearman $\rho$ with the
true error: the median over the 13 levels (or the 4 noise levels) for each seed, then the mean
over three seeds. "partial" controls for brightness; "pooled" mixes all levels, with the
severity-only baseline in parentheses; "flagged" is the share of the worst-quartile pixel-mask
errors whose residual is in its top quartile.

| model | error | $g$ | $\rho(g,e)$ within | noise | partial | $\rho(d,e)$ | pooled $\rho(g,e)$ | flagged |
|---|---|---|---|---|---|---|---|---|
| bottleneck, $\lambda_{id}=0$ | 0.092 | 0.015 | 0.70 | 0.74 | 0.59 | 0.77 | 0.55 (0.46) | 18% |
| bottleneck, $\lambda_{id}=0.1$ | 0.092 | 0.013 | 0.71 | 0.73 | 0.56 | 0.77 | 0.52 (0.41) | 13% |
| bottleneck, $\lambda_{id}=1$ | 0.095 | 0.009 | 0.62 | 0.60 | 0.53 | 0.83 | 0.49 (0.34) | 10% |
| bottleneck, $\lambda_{id}=1$, inner only | 0.097 | 0.014 | 0.61 | 0.68 | 0.61 | 0.82 | 0.53 (0.46) | 22% |
| skip, $\lambda_{id}=0$ | 0.086 | 0.019 | 0.86 | 0.92 | 0.48 | 0.49 | 0.52 (0.40) | 68% |
| skip, $\lambda_{id}=1$ | 0.084 | 0.007 | 0.63 | 0.60 | 0.27 | 0.28 | 0.40 (0.33) | 31% |

1. **Exact projectors carry no residual information.** A first-order expansion gives
   $g \approx \lvert J_f(y)\,(f(y)-y) \rvert$; for a projector the displacement is normal to
   the fixed-point set and the Jacobian removes it. Measured: $g < 2\cdot10^{-15}$ and the
   linearized term $10^{-13}$ (finite-difference round-off).
2. **For learned models the residual depends on the architecture.** The skip model, further
   from a projector, has a more informative residual under noise (0.92 against 0.74) and
   flags 68% of the worst pixel-mask errors against 18% for the bottleneck model. Its
   displacement is worse, and negative under noise ($-0.62$). Controlling for brightness
   reverses the order of the architectures (partial 0.48 against 0.59).
3. **Training for idempotence makes the residual a worse signal** in both architectures,
   with no overlap between seeds (0.70 to 0.62 and 0.86 to 0.63), while restoration barely
   changes. $\lambda_{id} = 0.1$ leaves it intact. Updating only the inner application, as in
   IGN, preserves most of it, so the damage comes mainly from the outer route.
4. **Without warm-up both architectures collapse** to a constant, exactly idempotent image:
   the per-pixel median image (error 0.21059, identical to that image's error) and a black
   image (0.287). The bottleneck model starts almost constant (output spread $5\cdot10^{-6}$),
   the skip model does not ($8\cdot10^{-3}$), so the start decides where a model collapses,
   not whether (`results/checks.json`).
5. **Calibration per level** lifts pooled $\rho$ from 0.02-0.55 to 0.85-0.94, but a
   severity-only prediction already reaches 0.73-0.84 (`results/calibration*`).
