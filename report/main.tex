...
changes it. We also use SURE~\cite{stein1981sure,ramani2008mcsure}, conformal error
bars~\cite{angelopoulos2021gentle,teneggi2023krcps,narnhofer2025surecp}, selective
risk~\cite{scrc2025}, and the claim that disagreement captures the epistemic uncertainty
self-consistency misses~\cite{disagreement2026}.
...
the orbit~\cite{durasov2024zigzag}. Ensemble disagreement
$\mathrm{dis}(y) = \tfrac{1}{M|\Omega|}\sum_i \norm{f_i(y) - \bar f(y)}_1$ over $M = 3$ seeds is
the epistemic counterpart self-consistency is argued to miss~\cite{disagreement2026}.
...
scaled so 0 is ranking by true error and 1 a constant score~\cite{scrc2025}.
...
fail the same way on an unseen operator~\cite{disagreement2026}. Rejecting the 20\% of images with
...
We added box masks, SURE, brightness and level baselines, a second architecture, routing
ablations, deeper iterates and ensemble disagreement; $\lid > 0$ needed a warm-up. Coverage is
marginal and per family, not per pixel. Evidence is from one small dataset, two small models, three
seeds per setting and one collapse run per architecture. Fashion-MNIST's black background makes
