...
def pixel_mask_recall(per_image):
    """Share of the worst-quartile errors under pixel masks whose g is in its own top quartile."""
    recalls = {}
    for model, frame in per_image.groupby("model", sort=False):
        ...
        target = worst & (frame["family"] == "pixel_mask")
        recalls[model] = flagged[target].mean()
    return pd.Series(recalls)
...
def main():
    ...
    statistics["pooled_g_severity"] = pooled[pooled["signal"] == "g@level"].set_index("model")["spearman"]
    statistics["pixel_recall_g"] = pixel_mask_recall(per_image)
    ...
    for group, architecture, training in VARIANTS:
        ...
        for column in ("error", "residual", "rho_g", "rho_g_noise", "partial_g", "rho_d", "rho_rA",
                       "rho_d_noise", "pooled_g", "pooled_g_severity", "pixel_recall_g"):
...
def latex_rows(table):
    ...
    lines = [
        ...
        r"\toprule",
        r"& & & \multicolumn{3}{c}{$\rho(g, e)$} & $\rho(d, e)$ & $\rho(r_A, e)$ & $\rho(g, e)$ & \\",
        r"\cmidrule(lr){4-6}",
        r"training & $e$ & $g$ & within & noise & partial & within & within & pooled & flagged \\",
        r"\midrule",
    ]
    for architecture in ("bottleneck", "skip"):
        rows = table[table["architecture"] == architecture]
        lines.append(rf"\multicolumn{{10}}{{@{{}}l}}{{\emph{{{architecture} model}}}} \\")
        for _, row in rows.iterrows():
            collapsed = "no warm-up" in row["training"]
            training = row["training"].replace("lambda", r"$\lid =$")
            cells = [training, number(row["error"], 3), number(row["residual"], 3)]
            if collapsed:
                cells += ["--"] * 7
            else:
                cells += [number(row[c]) for c in ("rho_g", "rho_g_noise", "partial_g", "rho_d", "rho_rA")]
                cells.append(f"{number(row['pooled_g'])} ({number(row['pooled_g_severity'])})")
                cells.append(f"{100 * row['pixel_recall_g']:.0f}\\%")
